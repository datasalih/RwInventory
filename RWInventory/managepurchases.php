...
if (isset($_SESSION["company_id"])) {
    ...
    // Fetch data from purchases table for the specific company_id
    $sql = "SELECT * FROM purchases WHERE company_id = '$company_id'";
    $result = mysqli_query($connect, $sql);

    // Delete purchase if the remove button is clicked
    if (isset($_POST['delete_purchase'])) {
        $purchase_id = $_POST['purchase_id'];

        $delete = "DELETE FROM purchases WHERE purchase_id = '$purchase_id'";
        $delete_result = mysqli_query($connect, $delete);

        if ($delete_result) {
            // Purchase deleted successfully
            header("Location: managepurchases.php"); // Redirect to the purchases page to reflect the changes
            exit();
        } else {
...
<body>
	...
	<div class="headerl">
    <h1 style="text-align: center; font-weight: bold; font-size:35px;">Manage Purchases</h1>
    <a href="addpurchases.php">
		<button style="margin-left: 290px; margin-bottom: 30px;">Add Purchase</button>
	  </a>
  ...
  <div class="container">
	...
	<table>
		<thead>
			<tr>
				...
				<th>Purchase no.<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>
        <th>Customer Name<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>
        ...
        <th>Quantity<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>
				<th>Total Purchase<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>

				<th>Date Time<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>
				<th>Paid Status<img src="images/filter_icon.png" width="10px" style="margin-left:10px;"></th>
				<th>Actions</th>
			</tr>
		</thead>
		<tbody>
    ...
    if (mysqli_num_rows($result) > 0) {
      ...
      while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>";
        echo "<a href='editpurchases.php?id=" . urlencode($id) . "''><button>Edit</button></a>";


        echo "<form action='managepurchases.php' method='POST' style='display:inline;'>
        <input type='hidden' name='purchase_id' value='" . $row["purchase_id"] . "'>
        <button type='submit' name='delete_purchase' style='background-color:red;'>Remove</button>
      </form>";
